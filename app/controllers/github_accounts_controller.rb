class GithubAccountsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @github_account.save
        ...
        format.json { render action: 'show', status: :created, location: @github_account }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to github_accounts_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def github_account_params
      params.require(:github_account).permit(:login, :token, :user_id)
    end
end
